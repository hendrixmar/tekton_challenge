...
async def post_product(product_body: ProductBase, session: AsyncSession = Depends(get_async_session), user: SystemUser = Depends(get_current_user)):
    crud_instance = SqlAlchemyRepository(session, Product)
...
async def put_product(product_id: int, product_body: ProductBase, session: AsyncSession = Depends(get_async_session)):
    crud_instance = SqlAlchemyRepository(session, Product)
...
async def patch_product(product_id: int, product_body: ProductBase, session: AsyncSession = Depends(get_async_session)):
    crud_instance = SqlAlchemyRepository(session, Product)
    result: Product = await crud_instance.update(product_id, product_body)
    return result
